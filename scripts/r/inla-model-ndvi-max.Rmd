...
## Subset data at 0 and 5 snow.auc, to remove bias from inflation induced by measurement method

The Blaesedalen S2 data contains a bias induced by the measurement
method, with significant inflation of snow.auc values at 0 and 5. Values
of 0 represent pixels which never had any snow in them at any point in
the period of observation, while values of 5 represent pixels which
contained 100% fractional snow cover at the first observation and 0%
fractional snow cover at the second observation. In reality,
observations with a snow.auc of 5 would be distributed between values of
0 and 5, and many of the observations of 0 snow.auc would have negative
values. To reduce the bias created by the inflation of 0 and 5 values,
we will try subsampling observations of 0 and 5 to an observation
density similar to that of the other snow.auc values.
...
ggplot() +
  geom_histogram(data = s2.bl, aes(x = snow.auc), binwidth = 1)

```

It seems like the other snow.auc bins in the range 0-5 have
approximately 75 observations each. Sample the data in the 0-1 and 4-5
ranges to have only 75 obs each.

```{r}
# 0-1
zero <- s2.bl %>% filter(snow.auc < 1) %>%
  sample_n(size = 75, replace = FALSE)

# 4-5
five <- s2.bl %>% filter(snow.auc > 4 & snow.auc <= 5) %>%
  sample_n(size = 75, replace = FALSE)

# Put the data back together
s2.bl.balanced <- s2.bl %>% filter(snow.auc > 1) %>%
  filter(snow.auc < 4 | snow.auc >= 6)

s2.bl.balanced <- rbind(zero, five, s2.bl.balanced)

s2.bl.balanced <- s2.bl %>% filter(snow.auc >= 1) %>% filter(snow.auc < 4 | snow.auc >= 6)

# Hist
ggplot() +
  geom_histogram(data = s2.bl.balanced, aes(x = snow.auc), binwidth = 1)
```
...
**Summary:** The mean slope of snow.auc is 0.001 (positive) and the
credible interval overlaps with 0.
...
**Jakob's summary:** The credible interval does not overlap with zero,
or, given the data and model the 95% interval of the posterior
distribution indicates a negative slope with a mean of -0.003.
...
The credible interval does not overlap with zero, or, given the data and
model the 95% interval of the posterior distribution indicates a
negative slope with a mean of -0.003.
...
It looks like there may be slight spatial autocorrelation remaining in
the residuals from their distribution in space. In the South East of the
plot, there are some obvious lines of spatially grouped residuals, which
look like they're following water courses.
...
The credible interval does not overlap with zero, or, given the data and
model the 95% interval of the posterior distribution indicates a
negative slope with a mean of -0.01.
...
The histogram of the residuals **has an uneven distribution**, it is
skewed slightly positive but does not appear far from a normal
