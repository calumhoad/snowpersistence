...
test <- s2.bl %>%
  mutate(group_snow = as.factor(ifelse(snow.auc < 1, 0, 
                             ifelse(snow.auc > 4 & snow.auc < 6, 5, NA))))
# Check violin plots of snow.auc
ggplot(data = test) +
  geom_violin(aes(x = group_snow, y = ndvi.max.doy), width = 1)
...
fit_matern <- function(shape_param, scale_param, range_site, site_data, return_model = F){
  ...
  log.range = list(initial = log(range_site), fixed = FALSE)
...
s2.kl_fit <- fit_matern(scale_param = 0.01,
                        ...
                        site_data = s2.kl,
                        return_model = T)
s2.kh_fit <- fit_matern(scale_param = 0.1,
                        shape_param = 400,
                        range_site = ranges_df$range[3], 
                        site_data = s2.kh,
                        return_model = T)
s30.bl_fit <- fit_matern(scale_param = 0.1,
                         shape_param = 75,
                         range_site = ranges_df$range[4], 
                         site_data = s30.bl,
                         return_model = T)
...
plot_results <- function(model_matern, site_data){
  ...
  preds_credible <- expand.grid(intercept = unlist(model_matern$summary.fixed[1, c(1,3,5)]), 
    ...
    group_by(snow.auc) %>% summarise(min_ndvi_pred = min(ndvi_pred), max_ndvi_doy_pred = max(ndvi_pred))
