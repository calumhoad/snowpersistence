...
# Blaesedalen LandsatTS 
lsat.data <- read_csv('../../data/landsat-ts/trends/bl-auto-7-trends.csv') %>%
  st_as_sf(coords = c('X', 'Y'), crs = 4326) %>%
  st_transform(crs = 32621)

st_crs(lsat.data)$units

# buffer square to polygon
lsat.poly <- st_buffer(lsat.data, dist = 15, endCapStyle = "SQUARE")


# plot to check
ggplot() + geom_sf(data = lsat.poly, aes(fill = lsat.poly$total.change))# + geom_sf(data = lsat.data)
...
check <- st_as_sf(lsat.snow.cover)
...
ggplot() +
  geom_boxplot(data = extracted.data, aes(snow.av, trend.cat))
