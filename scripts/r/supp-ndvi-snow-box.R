...
categorise_ndvi <- function(data) {
  data.filtered <- data %>% 
    group_by(id) %>% 
    filter(row_number() == 10 & !is.na(ndvi.max))
...
bl.p <- ndvi_box(bl, label.max = 15, label.step = 5)
kl.p <- ndvi_box(kl, label)
kh.p <- ndvi_box(kh)
bl30.p <- ndvi_box(bls30)

combined <- plot_grid(kl.p, kh.p, bl.p, bl30.p, nrow = 2, ncol = 2, labels = c('BL', 'KL', 'KH', 'HLSS30 BL'))
