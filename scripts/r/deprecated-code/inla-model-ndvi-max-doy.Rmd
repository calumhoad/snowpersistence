---
title: "ndvi.max.doy.doy ~ snow.auc INLA"
...
positive slope with a mean of 0.202.
...
input.data$residuals <- input.data$ndvi.max.doy - model_matern$summary.linear.predictor$mean
```
...
doy.snow.lm <- lm(input.data$ndvi.max.doy ~ input.data$snow.auc)
...
ggplot() +
  geom_sf(data = st_buffer(input.data, 5, endCapStyle = "SQUARE"), 
          aes(fill = ndvi.max.doy - preds - residuals)) +
  scale_fill_continuous_diverging() +
...
The credible interval does not overlap with zero, or, given the data and
model the 95% interval of the posterior distribution indicates a
positive slope with a mean of 0.425.
...
The credible interval does not overlap with zero, or, given the data and
model the 95% interval of the posterior distribution indicates a
positive slope with a mean of 0.408.
...
complete_grid_plot <- ggplot() +
  ...
  theme_map()
kl_node_plot <- ggplot() +
  geom_sf(data = st_buffer(st_geometry(input.data), 15, endCapStyle = "SQUARE"),
          fill = NA) +
  geom_sf_text(data = input.data, 
               aes(label = node),
               size = 1) +
...
The credible interval does not overlap with zero, or, given the data and
model the 95% interval of the posterior distribution indicates a
positive slope with a mean of 1.399.
...
Something odd is happening here - I think the spatial effect is failing
to fit and the result is that the model does not account for the spatial
structure of the data. n_row and n_col are correct for the data, it may
be that the mean number of nodes (7.8) is too large?
